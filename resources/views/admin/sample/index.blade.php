                ...
                <div class="col-md-12">
                  <table class="table">
                    ...
                    <tbody>
                      ...
                      @php $i = $result['i']; $i < count($result['sampledata']); @endphp @foreach ($result['sampledata'] as $sampledata ) <tr>
                        <th>{{ $i++ }}</th>
                        ...
                        <td>
                          ...
                          <button class="btn btn-danger btn-xs hapus-data" data-toggle="modal" data-id="{{ $sampledata->id }}"><i class="fas fa-trash"></i></button>
          ...
          <form action="{{ route('sample.add') }}" method="post">
            ...
            <div class="form-group">
              ...
              <select name="tingkat" class="form-control @error(" tingkat") is-invalid @enderror">
                @foreach ($result['tingkat_data'] as $tingkat_data)
            ...
            <div class="form-group">
              ...
              <select name="pelatihan" class="form-control @error(" pelatihan") is-invalid @enderror">
                @foreach ($result['pelatihan_data'] as $item)
                <option value="{{$item->id_pelatihan}}" @if (old('pelatihan')==$item->id_pelatihan) selected @endif>{{$item->pelatihan}}</option>
                @endforeach
              </select>
              @error('pelatihan')
              <small class="text-danger">
