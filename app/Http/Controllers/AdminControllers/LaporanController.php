...
class LaporanController extends Controller
{
    ...
    public function rekomendasi(Request $request)
    {
        ...
        //$result['rekomendasidata'] =   $this->rekomendasi->rekomendasi_display()->$pelatihans;
        ...
        if ($pelatihans != '') 
            {
                $rekomendasidata=DB::table('rekomendasi as A')
								-> select('A.nik', 'G.nama', 'G.alamat', 'G.jenis_kelamin', 'A.rekomendasi',
                                  'B.tingkat', 
								  'C.jurusan', 
                                  'D.kelompok')
                -> leftjoin('tenagakerja as G', function($join) {$join->on('A.nik', '=', 'G.nik');
                                  })
								-> leftjoin('tingkat as B', function($join) {$join->on('G.id_tingkat', '=', 'B.id_tingkat');
                				   })
								-> leftjoin('jurusan as C', function($join) {$join->on('G.id_jurusan', '=', 'C.id_jurusan');
                				   })
								 -> leftjoin('kelompok as D', function($join) {$join->on('G.id_kelompok', '=', 'D.id_kelompok');
                				   })
								-> where('A.rekomendasi', '=',$pelatihans)
								-> orderBy('G.nama','DESC')
								-> get();
